...
wire1 = ['R75,D30,R83,U83,L12,D49,R71,U75,L72'];
...
% create functions over position

function [function1, cumlen1] = calcFunc(wire1);
startPos1 = [0, regexp(wire1,',')];
length1 = length(startPos1);
function1 = zeros(2,length1);
cumlen1 = 0;
for i = 1:length1
  dir = wire1(startPos1(i)+1);
  if(i == length1)
    len = str2num(wire1(startPos1(i)+2:end));
  else
    len = str2num(wire1(startPos1(i)+2:startPos1(i+1)-1));
  end
  cumlen1(i+1) = cumlen1(i) + len;
  
  if(dir == 'R')
    function1(1,i+1) = function1(1,i) + len;
    function1(2,i+1) = function1(2,i);
  elseif(dir == 'L')
    function1(1,i+1) = function1(1,i) - len;
    function1(2,i+1) = function1(2,i);
  elseif(dir == 'U')
    function1(1,i+1) = function1(1,i);
    function1(2,i+1) = function1(2,i) + len;
  elseif(dir == 'D')
    function1(1,i+1) = function1(1,i);
    function1(2,i+1) = function1(2,i) - len;
  end
...
[f1 cumlen1] = calcFunc(wire1);
[f2 cumlen2] = calcFunc(wire2);
plot(f1(1,:),f1(2,:))
hold on;
plot(f2(1,:),f2(2,:))

% find common coordinates
function [crossCounter,encounters,encounterLength,coordinates] = findIntersection(f1,f2,cumlen1,cumlen2)
crossCounter = 0;
coordinates = [];
encounters = [];
encounterLength = [];
for i = 2:(length(f1)-1)
 for j = 2:(length(f2)-1)
    if(f1(1,i) == f1(1,i+1))% verticaal f1, x equal
    maxY = max(f1(2,i),f1(2,i+1));
    minY = min(f1(2,i),f1(2,i+1));
      if(f2(2,j) > minY && f2(2,j) < maxY && f2(2,j+1) > minY && f2(2,j+1) < maxY)
        if((f2(1,j) < f1(1,i) && f2(1,j+1) > f1(1,i)) || (f2(1,j) > f1(1,i) && f2(1,j+1) < f1(1,i)))
          crossCounter +=1;
          encounters(1,crossCounter) = i;
          encounters(2,crossCounter) = j;
          encounterLength(1,crossCounter) = cumlen1(i) + abs(f1(2,i)-f2(2,j));
          encounterLength(2,crossCounter) = cumlen2(j) + abs(f1(1,i)-f2(1,j));
          coordinates(1,crossCounter) = f1(1,i);
          coordinates(2,crossCounter) = f2(2,j);
        end
      end
    else % horizontal f1
      maxX = max(f1(1,i),f1(1,i+1));
      minX = min(f1(1,i),f1(1,i+1));
      if(f2(1,j) > minX && f2(1,j) < maxX && f2(1,j+1) > minX && f2(1,j+1) < maxX)
        if((f2(2,j) < f1(2,i) && f2(2,j+1) > f1(2,i)) || (f2(2,j) > f1(2,i) && f2(2,j+1) < f1(2,i)))
          crossCounter +=1;
          encounters(1,crossCounter) = i;
          encounters(2,crossCounter) = j;
          encounterLength(1,crossCounter) = cumlen1(i) + abs(f2(1,j)-f1(1,i));
          encounterLength(2,crossCounter) = cumlen2(j) + abs(f2(2,j)-f1(2,i));
          coordinates(1,crossCounter) = f2(1,j);
          coordinates(2,crossCounter) = f1(2,i);
        end
      end
    end
 end
...
[crossCounter,encounters,encounterLength,coordinates] = findIntersection(f1,f2,cumlen1,cumlen2);

% find shortest distance
for i = 1:length(coordinates)
  distance(i) = abs(coordinates(1,i)) + abs(coordinates(2,i));
  wireLengths(i) = encounterLength(1,i) + encounterLength(2,i);
end
distance;
min(distance);
wireLengths;
...
% ? niet nodig
[crossCounterFun1,encountersFun1,~,coordinatesFun1] = findIntersection(f1,f1,cumlen1,cumlen1);
[crossCounterFun2,encountersFun2,~,coordinatesFun2] = findIntersection(f2,f2,cumlen2,cumlen2);

% herbereken minimale route per 1-wire loop
for i = 1:length(encountersFun1)/2
